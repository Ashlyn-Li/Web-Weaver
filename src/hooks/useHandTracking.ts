import { useEffect, useRef, useState, type RefObject } from 'react'
import type {
  HandLandmarker,
  HandLandmarkerResult,
} from '@mediapipe/tasks-vision'
import { createHandLandmarker } from '../vision/createHandLandmarker'
import { WEB_RENDERING } from '../config/web'
import {
  InteractionProcessor,
  createEmptyInteractionSnapshot,
} from '../gestures/processInteractions'
import type { ProcessedHand } from '../types/hand'
import type { InteractionSnapshot } from '../types/interaction'
import type { WebGraph } from '../types/web'
import type {
  HandTrackingStatus,
  HandTrackingSummary,
} from '../types/handTracking'
import { HandProcessor } from '../vision/processHands'
import { WebGenerator } from '../web/generateWeb'

const emptySummary: HandTrackingSummary = {
  handCount: 0,
  leftHandDetected: false,
  rightHandDetected: false,
}

const maxConsecutiveInferenceErrors = 5

function getTrackingErrorMessage(error: unknown) {
  if (error instanceof Error) {
    return error.message
  }

  return 'Unexpected hand-tracking failure'
}

function getSummaryFromResult(result: HandLandmarkerResult): HandTrackingSummary {
  const handedness = result.handedness ?? result.handednesses ?? []

  return handedness.reduce<HandTrackingSummary>(
    (summary, handClassifications) => {
      const label = handClassifications[0]?.categoryName.toLowerCase()

      // The preview video is mirrored in CSS only; MediaPipe receives the
      // unmirrored video frame, so its handedness labels are used as reported.
      return {
        handCount: summary.handCount + 1,
        leftHandDetected: summary.leftHandDetected || label === 'left',
        rightHandDetected: summary.rightHandDetected || label === 'right',
      }
    },
    {
      handCount: result.landmarks.length,
      leftHandDetected: false,
      rightHandDetected: false,
    },
  )
}

function summariesMatch(
  currentSummary: HandTrackingSummary,
  nextSummary: HandTrackingSummary,
) {
  return (
    currentSummary.handCount === nextSummary.handCount &&
    currentSummary.leftHandDetected === nextSummary.leftHandDetected &&
    currentSummary.rightHandDetected === nextSummary.rightHandDetected
  )
}

type UseHandTrackingResult = {
  status: HandTrackingStatus
  summary: HandTrackingSummary
  interactionSnapshot: InteractionSnapshot
  processedHands: readonly ProcessedHand[]
  webGraph: WebGraph | null
  error: string | null
  latestResultRef: RefObject<HandLandmarkerResult | null>
  interactionSnapshotRef: RefObject<InteractionSnapshot>
  processedHandsRef: RefObject<readonly ProcessedHand[]>
  webGraphRef: RefObject<WebGraph | null>
}

export function useHandTracking(
  videoRef: RefObject<HTMLVideoElement | null>,
  isActive: boolean,
): UseHandTrackingResult {
  const [status, setStatus] = useState<HandTrackingStatus>('idle')
  const [summary, setSummary] = useState<HandTrackingSummary>(emptySummary)
  const [interactionSnapshot, setInteractionSnapshot] =
    useState<InteractionSnapshot>(createEmptyInteractionSnapshot)
  const [processedHands, setProcessedHands] = useState<readonly ProcessedHand[]>([])
  const [webGraph, setWebGraph] = useState<WebGraph | null>(null)
  const [error, setError] = useState<string | null>(null)
  const detectorRef = useRef<HandLandmarker | null>(null)
  const frameIdRef = useRef<number | null>(null)
  const latestResultRef = useRef<HandLandmarkerResult | null>(null)
  const interactionProcessorRef = useRef(new InteractionProcessor())
  const interactionSnapshotRef = useRef<InteractionSnapshot>(
    createEmptyInteractionSnapshot(),
  )
  const processedHandsRef = useRef<readonly ProcessedHand[]>([])
  const webGeneratorRef = useRef(new WebGenerator())
  const webGraphRef = useRef<WebGraph | null>(null)
  const processorRef = useRef(new HandProcessor())
  const lastVideoTimeRef = useRef(-1)
  const summaryRef = useRef<HandTrackingSummary>(emptySummary)

  const updateProcessedHands = (nextHands: readonly ProcessedHand[]) => {
    const currentHands = processedHandsRef.current
    const changed =
      currentHands.length !== nextHands.length ||
      currentHands.some((hand, index) => {
        const nextHand = nextHands[index]

        return (
          !nextHand ||
          hand.id !== nextHand.id ||
          hand.fingerStates.thumb.extended !== nextHand.fingerStates.thumb.extended ||
          hand.fingerStates.index.extended !== nextHand.fingerStates.index.extended ||
          hand.fingerStates.middle.extended !== nextHand.fingerStates.middle.extended ||
          hand.fingerStates.ring.extended !== nextHand.fingerStates.ring.extended ||
          hand.fingerStates.little.extended !== nextHand.fingerStates.little.extended
        )
      })

    processedHandsRef.current = nextHands

    if (changed) {
      setProcessedHands(nextHands)
    }
  }

  const updateInteractionSnapshot = (nextSnapshot: InteractionSnapshot) => {
    const currentSnapshot = interactionSnapshotRef.current
    const changed =
      currentSnapshot.stableInteraction.current !==
        nextSnapshot.stableInteraction.current ||
      currentSnapshot.stableInteraction.confidence !==
        nextSnapshot.stableInteraction.confidence ||
      currentSnapshot.stableInteraction.activeHandId !==
        nextSnapshot.stableInteraction.activeHandId ||
      currentSnapshot.stableHandGestures.length !==
        nextSnapshot.stableHandGestures.length ||
      currentSnapshot.stableHandGestures.some((gesture, index) => {
        const nextGesture = nextSnapshot.stableHandGestures[index]

        return (
          !nextGesture ||
          gesture.handId !== nextGesture.handId ||
          gesture.gesture !== nextGesture.gesture ||
          gesture.confidence !== nextGesture.confidence
        )
      }) ||
      currentSnapshot.webAnchorCandidates.length !==
        nextSnapshot.webAnchorCandidates.length

    interactionSnapshotRef.current = nextSnapshot

    if (changed) {
      setInteractionSnapshot(nextSnapshot)
    }
  }

  const updateWebGraph = (
    nextSnapshot: InteractionSnapshot,
    nextHands: readonly ProcessedHand[],
  ) => {
    const nextGraph = WEB_RENDERING.enabled
      ? webGeneratorRef.current.generate({
          interaction: nextSnapshot.stableInteraction,
          processedHands: nextHands,
          anchorCandidates: nextSnapshot.webAnchorCandidates,
        })
      : null
    const currentGraph = webGraphRef.current
    const changed =
      currentGraph?.mode !== nextGraph?.mode ||
      currentGraph?.anchors.length !== nextGraph?.anchors.length ||
      currentGraph?.strands.length !== nextGraph?.strands.length

    webGraphRef.current = nextGraph

    if (changed) {
      setWebGraph(nextGraph)
    }
  }

  useEffect(() => {
    const interactionProcessor = interactionProcessorRef.current
    const processor = processorRef.current
    const webGenerator = webGeneratorRef.current

    if (!isActive) {
      setStatus('idle')
      setSummary(emptySummary)
      setInteractionSnapshot(createEmptyInteractionSnapshot())
      setProcessedHands([])
      setWebGraph(null)
      setError(null)
      summaryRef.current = emptySummary
      latestResultRef.current = null
      interactionSnapshotRef.current = createEmptyInteractionSnapshot()
      interactionProcessor.reset()
      processedHandsRef.current = []
      webGraphRef.current = null
      processor.reset()
      webGenerator.reset()
      return
    }

    let isCancelled = false
    let consecutiveInferenceErrors = 0

    const stopLoop = () => {
      if (frameIdRef.current !== null) {
        cancelAnimationFrame(frameIdRef.current)
        frameIdRef.current = null
      }
    }

    const runDetectionLoop = () => {
      if (isCancelled) {
        return
      }

      const video = videoRef.current
      const detector = detectorRef.current

      if (
        !video ||
        !detector ||
        video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA
      ) {
        frameIdRef.current = requestAnimationFrame(runDetectionLoop)
        return
      }

      if (video.currentTime !== lastVideoTimeRef.current) {
        lastVideoTimeRef.current = video.currentTime

        try {
          const result = detector.detectForVideo(video, performance.now())
          latestResultRef.current = result
          const nextHands = processor.process(result)
          updateProcessedHands(nextHands)
          const nextInteractionSnapshot = interactionProcessor.process(nextHands)
          updateInteractionSnapshot(nextInteractionSnapshot)
          updateWebGraph(nextInteractionSnapshot, nextHands)
          consecutiveInferenceErrors = 0

          const nextSummary = getSummaryFromResult(result)

          if (!summariesMatch(summaryRef.current, nextSummary)) {
            summaryRef.current = nextSummary
            setSummary(nextSummary)
          }

          setStatus(nextSummary.handCount > 0 ? 'tracking' : 'ready')
        } catch (caughtError) {
          consecutiveInferenceErrors += 1
          console.error('Hand-tracking inference failed', caughtError)

          if (consecutiveInferenceErrors >= maxConsecutiveInferenceErrors) {
            setStatus('error')
            setError('Unexpected hand-tracking failure')
            stopLoop()
            return
          }
        }
      }

      frameIdRef.current = requestAnimationFrame(runDetectionLoop)
    }

    const initialiseTracking = async () => {
      setStatus('loading-model')
      setError(null)
      setSummary(emptySummary)
      setInteractionSnapshot(createEmptyInteractionSnapshot())
      setProcessedHands([])
      setWebGraph(null)
      summaryRef.current = emptySummary
      latestResultRef.current = null
      interactionSnapshotRef.current = createEmptyInteractionSnapshot()
      interactionProcessor.reset()
      processedHandsRef.current = []
      webGraphRef.current = null
      webGenerator.reset()
      lastVideoTimeRef.current = -1

      try {
        const detector = await createHandLandmarker()

        if (isCancelled) {
          detector.close()
          return
        }

        detectorRef.current = detector
        setStatus('ready')
        frameIdRef.current = requestAnimationFrame(runDetectionLoop)
      } catch (caughtError) {
        if (isCancelled) {
          return
        }

        setStatus('error')
        setError(getTrackingErrorMessage(caughtError))
      }
    }

    void initialiseTracking()

    return () => {
      isCancelled = true
      stopLoop()
      detectorRef.current?.close()
      detectorRef.current = null
      latestResultRef.current = null
      interactionSnapshotRef.current = createEmptyInteractionSnapshot()
      interactionProcessor.reset()
      processedHandsRef.current = []
      webGraphRef.current = null
      webGenerator.reset()
      processor.reset()
    }
  }, [isActive, videoRef])

  return {
    status,
    summary,
    interactionSnapshot,
    processedHands,
    webGraph,
    error,
    latestResultRef,
    interactionSnapshotRef,
    processedHandsRef,
    webGraphRef,
  }
}
